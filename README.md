...
O checker arquitetural `0.2.0` continua produzindo `fail` comprovável para violações de direção de dependência (`ARCH-002`) e acesso a superfícies internas (`MOD-001`), mantendo ausência de finding como `unknown`. Coverage attestation/composition e ecosystem discovery/alignment provam suficiência e escopo; o trusted result aggregator agora consegue verificar evidência positiva para ARCH-002/MOD-001 dentro do grafo de dependências efetivamente observado. Como os adapters atuais não provam toda forma semanticamente possível de dependência arquitetural, `complete_rule_semantics` permanece `false` e o status normativo continua `unknown` na ausência de `fail`. A cobertura positiva ainda está limitada a Python e ECMAScript/TypeScript sem JSX e o mecanismo agregado ainda não está integrado ao comando final de adoção/enforcement; por isso ARCH-002 e MOD-001 permanecem `fail_only`/`not_ready`, e nenhuma das quatro regras está `active_ready`.
