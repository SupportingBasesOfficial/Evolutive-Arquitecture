...
O checker arquitetural `0.2.0` continua produzindo `fail` comprovável para violações de direção de dependência (`ARCH-002`) e acesso a superfícies internas (`MOD-001`), mantendo ausência de finding como `unknown`. Coverage attestation/composition e ecosystem discovery/alignment provam suficiência e escopo; o trusted result aggregator consegue verificar evidência positiva dentro do grafo observado, e a rule semantic coverage agora mostra mecanicamente quais classes da semântica da regra estão cobertas, parciais ou descobertas. Como taxonomia/profile ainda não são exaustivos e os adapters atuais cobrem apenas uma fração das relações conhecidas, `complete_rule_semantics` permanece estruturalmente `false`; ARCH-002 e MOD-001 continuam `fail_only`/`not_ready`, e nenhuma das quatro regras está `active_ready`.
