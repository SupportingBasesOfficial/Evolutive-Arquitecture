...
A [provenance producer trust](./docs/PROVENANCE_PRODUCER_TRUST.md) separa raw evidence da confiança no código que o produziu. Há um producer declarativo e um producer observado: o segundo lê apenas um `build_manifest` brokerado, valida seu SHA-256 e os artifact bindings autorizados e emite `observation_basis: observed` sem executar build ou código do consumidor. O trust attestor v0.2.0 reproduz ambos e mantém `trust_only`: `verified + observed` comprova leitura confiável de fatos materializados no artefato brokerado, não relação semântica comprovada nem `rule-pass`.
