...
A [coverage attestation](./docs/COVERAGE_ATTESTATION.md) verifica se a evidência é reproduzível no snapshot atual e se a observação é suficientemente completa dentro do escopo exato do manifesto do adapter. Essa camada continua separada do checker: `sufficient` ainda não significa conformidade normativa positiva.
...
O [ecosystem discovery e observation alignment](./docs/ECOSYSTEM_DISCOVERY_ALIGNMENT.md) compara o snapshot autorizado contra um catálogo governado de superfícies de código. Superfícies conhecidas sem adapter e observations detectadas mas omitidas na policy impedem `aligned`; arquivos fora do catálogo permanecem explicitamente `unclassified`. Essa camada continua `catalog_scope_only` e não produz conformidade normativa positiva.

O [trusted result aggregator](./docs/TRUSTED_RESULT_AGGREGATION.md) regenera alignment, coverage, evidências e checker results frescos e pode registrar `positive_evidence: verified` somente para profiles explicitamente governados. Essa afirmação é limitada ao `observed_dependency_graph`: o aggregator v0.1.0 não possui autoridade de rule-pass, não muta o checker e não altera status normativo de regra.

A [rule semantic coverage](./docs/RULE_SEMANTIC_COVERAGE.md) decompõe a semântica conhecida de ARCH-002/MOD-001 em classes de relação arquitetural e compara essas classes com as capabilities reais dos adapters no snapshot. Hoje `source_module_dependency` pode ser coberta por Python e ECMAScript quando suas attestations são `sufficient`, enquanto construction, dynamic resolution, FFI, IPC, convenções comportamentais e outras relações permanecem explicitamente não cobertas. Taxonomia e profiles continuam `not_established` quanto à exaustividade e o evaluator v0.1.0 fixa `complete_rule_semantics: false`.

A [governança de exaustividade semântica](./docs/SEMANTIC_EXHAUSTIVENESS_GOVERNANCE.md) define como uma futura alegação `established` deve ser decidida. Toda aprovação precisa carregar snapshot semântico imutável, SHA-256 canônico, revisão multidimensional, busca adversarial por contraexemplos e evidências independentes; decisões approved com gaps, contraexemplos unresolved ou aprovação dormente são recusadas pelo gate. Nesta versão nenhuma exaustividade foi aprovada e nenhuma autoridade de rule-pass foi criada.

A [review evidence de exaustividade semântica](./docs/SEMANTIC_EXHAUSTIVENESS_REVIEW_EVIDENCE.md) separa material de revisão da decisão. Taxonomy, ARCH-002 e MOD-001 possuem pacotes current-review vinculados por SHA ao snapshot semântico exato; os três concluem `inconclusive` e preservam como gaps explícitos macros/metaprogramação, generated code e build-time linkage. O gate invalida automaticamente uma review stale quando taxonomy/profile muda e nenhuma conclusão de review altera status ou produz `rule-pass`.

A [governança de build-time provenance](./docs/BUILD_TIME_PROVENANCE.md) torna representáveis dependências introduzidas por macros, generated source, transforms de AST/IR, compiler plugins, build graphs, linkers e packaging. Provenance e semântica permanecem autoridades separadas: cada origem possui apenas `candidate_relations` com mapping estruturalmente `partial`, e o evidence schema vincula inputs/outputs por SHA-256 sem poder declarar relação semântica comprovada, exaustividade ou `rule-pass`.

A [provenance producer trust](./docs/PROVENANCE_PRODUCER_TRUST.md) separa raw evidence da confiança no código que o produziu. O primeiro producer de referência verifica declarações contra artifact bindings autorizados de forma determinística, sem rede, subprocessos, environment ou execução de código do consumidor. Uma trust attestation independente reexecuta o producer e vincula declaration, authorized artifacts, evidence, producer manifest, implementação e trust attestor por SHA-256. O producer atual permanece `observation_basis: declared`: `verified` comprova reprodutibilidade do verificador, não provenance observada nem relação semântica comprovada.
...
O checker arquitetural `0.2.0` continua produzindo `fail` comprovável para violações de direção de dependência (`ARCH-002`) e acesso a superfícies internas (`MOD-001`), mantendo ausência de finding como `unknown`. Coverage attestation/composition e ecosystem discovery/alignment provam suficiência e escopo; o trusted result aggregator consegue verificar evidência positiva dentro do grafo observado, e a rule semantic coverage mostra mecanicamente quais classes da semântica da regra estão cobertas, parciais ou descobertas. Build-time provenance agora torna macros/generated code/linkage representáveis e já existe um producer determinístico confiável para validar **declarações** contra bindings autorizados, mas ainda faltam producers confiáveis com `observation_basis: observed` e mapeamento semanticamente completo; taxonomy/profile permanecem `not_established`, as reviews atuais são `inconclusive`, nenhum registro approved existe e `complete_rule_semantics` continua estruturalmente `false`; ARCH-002 e MOD-001 seguem `fail_only`/`not_ready`, e nenhuma das quatro regras está `active_ready`.
