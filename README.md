...
A [review evidence de exaustividade semântica](./docs/SEMANTIC_EXHAUSTIVENESS_REVIEW_EVIDENCE.md) separa material de revisão da decisão. Taxonomy, ARCH-002 e MOD-001 agora possuem pacotes current-review vinculados por SHA ao snapshot semântico exato; os três concluem `inconclusive` e preservam como gaps explícitos macros/metaprogramação, generated code e build-time linkage. O gate invalida automaticamente uma review stale quando taxonomy/profile muda e nenhuma conclusão de review altera status ou produz `rule-pass`.
