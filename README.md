...
O checker arquitetural `0.2.0` já consegue produzir `fail` comprovável para violações de direção de dependência (`ARCH-002`) e acesso a superfícies internas (`MOD-001`) quando recebe um grafo arquitetural validado. Os adapters de referência Python e ECMAScript já produzem esse grafo a partir de relações resolvidas conservadoramente, mas a cobertura continua específica de ecossistema e incompleta diante de ambiguidades. A ausência de achados continua `unknown`; portanto nenhuma das quatro regras está `active_ready`.
